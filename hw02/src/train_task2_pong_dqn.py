        ...
        for ep in tqdm(range(episodes), desc="Training"):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                if self.env_count % 1000 == 0:                 
                    print(f"[Collect] Ep: {ep + 1} Step: {step_count} SC: {self.env_count} UC: {self.train_count} Eps: {self.epsilon:.4f}")
                    wandb.log({
                        "Episode": ep,
                    ...
                    ########## END OF YOUR CODE ##########   
            print(f"[Eval] Ep: {ep + 1} Total Reward: {total_reward} SC: {self.env_count} UC: {self.train_count} Eps: {self.epsilon:.4f}")
            wandb.log({
                "Episode": ep,
            ...
            ########## END OF YOUR CODE ##########  
            if ep + 1 % 100 == 0:
                model_path = os.path.join(self.save_dir, f"model_ep{ep + 1}.pt")
                torch.save(self.q_net.state_dict(), model_path)
                print(f"Saved model checkpoint to {model_path}")

            if (ep  + 1) % 20 == 0 and ep + 1 > 500:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", type=str, default="./outputs/results/task2")
    parser.add_argument("--wandb-run-name", type=str, default="task2")
    ...
    wandb.init(project="DLP-hw02-Task2-Pong", name=args.wandb_run_name, save_code=True)
